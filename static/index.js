...
function showGameScoreLayer() {
    ...
    document.getElementById('GameScoreLayer-image').src = shareImage(_gameScore);
    var score_text='得分&nbsp;&nbsp;';
    ...
    document.getElementById('GameScoreLayer-best').innerHTML = '最佳&nbsp;&nbsp;' + bast;
    l.style.display = 'block';
}
...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 49) return '你算是寄了！';
    if (score <= 99) return '你不会寄了吧！';
    if (score <= 149) return '要寄了吗？';
    if (score <= 199) return '差一点点就要寄了呢';
    return '飞鹰子爱你呦~';
}

function shareImage(score) {
    if (score <= 49) return './static/image/1.jpg';
    if (score <= 99) return './static/image/2.jpg';
    if (score <= 149) return './static/image/3.jpg';
    if (score <= 199) return './static/image/4.gif';
    return './static/image/5.jpg';
}
